...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw , I, SEXT(BITS(R(dest) = (word_t)src1>>BITS(src2,4,0),31,0),32));  
  INSTPAT("?1????? ????? ????? 101 ????? 00100 11", srai  , I, R(dest) = (sword_t)src1>>BITS(src2,5,0));
  INSTPAT("?1????? ????? ????? 101 ????? 00110 11", sraiw , I, R(dest) = SEXT(BITS((sword_t)src1>>BITS(src2,5,0),31,0),32));
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add   , R, R(dest) = src1+src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub   , R, R(dest) = src1-src2);
